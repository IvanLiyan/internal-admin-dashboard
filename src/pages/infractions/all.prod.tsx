...
const AllInfractionsPage: NextPage<Record<string, never>> = () => {
  ...
  const [orderBy] =
    useState<(typeof AllInfractionTableColumns)[number]>("lastUpdated");
  const [selected, setSelected] = useState<readonly string[]>([]);
  const [page, setPage] = useState(0);
  const [limit, setLimit] = useState(10);
  const [states] = useState<MerchantWarningState[]>([
    "AWAITING_MERCHANT",
    "NEW",
    "CLOSED",
    "AWAITING_ADMIN",
  ...
  const tableData = useInfractionTableData(AllInfractionTableColumns, data);
